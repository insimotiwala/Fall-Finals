...
public class MobileUnit : MonoBehaviour
{
    ...
    public void StartConfigure()
    {
        ...
        _reachedTarget = true;
        //Agent.enabled = false;
        GameObject keys1 = Configure.Keys.ToList()[0];
        Configure[keys1] = this.gameObject;
    }
    ...
    private void Update()
    {
        if (_reachedTarget)
        {
            ...
            foreach (KeyValuePair<GameObject, GameObject> kvp in Configure)
            {
                ...
                // Debug.Log(pos);
                Debug.DrawLine(cAgent.transform.position, pos, Color.red);
                // cAgent.transform.position = Vector3.Lerp(cAgent.transform.position, pos, Time.deltaTime * speed);
                Vector3 moveDirection = pos - cAgent.transform.position;
                cAgent.transform.position += moveDirection.normalized * MoveDistance * Time.deltaTime;
                cAgent.transform.rotation = Quaternion.Lerp(cAgent.transform.rotation, rot, Time.deltaTime * speed);
                if (Vector3.Distance(cAgent.transform.position, pos) < 0.1f)
                {
                    Debug.Log("reached");
                    cAgent.transform.position = pos;
                    cAgent.transform.rotation = rot;
                    cAgent.transform.parent = kvp.Key.transform;
